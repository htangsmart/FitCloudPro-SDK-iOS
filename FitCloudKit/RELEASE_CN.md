...
- 新增对 212×520 方形表盘的支持。
- 新增对 410×480 方形表盘的支持。
- 修复了 HRV 定时测量设置漏掉【启用提示】的问题。
- 修改了 ANCS 通知应用配置相关的 API。
...
- 新增标志位，用于标识手表是否支持 App 助眠分析。
- 新增标志位，用于标识手表是否需要在 App 上隐藏时间制式 (24 小时制/12 小时制) 设置入口。
- 设备密码长度现在允许根据设备而变化。
- 新增补充运动定义。
...
- 新增在支持的设备上通过 Opus 传输播放 PCM 音频的 API。
- 新增 SDK 初始化选项，默认自动纠正不合理的用户资料数值；如不需要自动纠正，须主动关闭该选项。
- 新增设备文件拉取、Opus 音频播放、离线地图等固件能力标志。
- 添加对 410×494 方形表盘的支持。
- 重构公开 AI API，采用按业务划分的协同会话，并新增完整的集成指南。
- 修复单轮 AI 语音会话的回调顺序，避免将自然结束误报为取消，并确保中断回调先于退出回调。
- 扩展设备能力上报，支持获取多图、弹幕、视频及旋转角度等表盘能力，以及录音暂停/恢复和 ASR 文本要求。
- 优化 OTA 传输流程，提升升级过程的稳定性与可靠性。
- 新增支持在兼容手表上下载、传输和管理离线地图。
- 修复了 `queryTasksWithCompletion:` 返回任务数据乱码的问题。
- 新增弹幕发送与清除 API，支持向设备发送和清除弹幕（绿联定制）。
- 优化设备断开与回连期间的蓝牙命令处理，确保待处理请求正确结束，并保持多包响应顺序。
